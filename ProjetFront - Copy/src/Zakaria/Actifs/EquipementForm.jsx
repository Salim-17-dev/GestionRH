...
import { Building, Hash, FileText, CreditCard, Calendar } from "lucide-react";
import {
...
const EquipementForm = ({
  ...
  const isValid = useMemo(() => {
    return Boolean(formData.designation && formData.categorie && formData.numero_serie && formData.etat && formData.statut);
  }, [formData]);
  ...
  const validateForm = () => {
    const errors = {};
    if (!formData.designation.trim()) errors.designation = "La designation est requise";
    if (!formData.categorie) errors.categorie = "La categorie est requise";
    if (!formData.numero_serie.trim()) errors.numero_serie = "Le numero de serie est requis";
    if (!formData.etat) errors.etat = "L'etat est requis";
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        valeur: formData.valeur === "" ? null : Number(formData.valeur),
    ...
    <>
      ...
      <Card className="societe-form-container" style={{ height: "100%", width: "100%" }}>
        <div className="societe-form-header">
          <h5>{initialData ? "Modifier Equipement" : "Ajouter Equipement"}</h5>
        </div>

        <div className="societe-form-body">
          <Form onSubmit={handleSubmit}>
            {!HIDDEN_FORM_FIELDS.has("photo") && (
              <div className="form-group-wrapper">
                <Form.Label className="form-label-enhanced">
                  Photo (Optionnel)
                </Form.Label>
                <div className="photo-picker-wrapper">
                  <label className={`photo-picker ${formData.photo ? "has-file" : ""}`}>
                    <input
                      type="file"
                      name="photo"
                      accept="image/*"
                    ...
                    <div className="photo-picker-content">
                      {formData.photo instanceof File ? (
                        <div className="photo-preview">
                          <img src={URL.createObjectURL(formData.photo)} alt="Preview" className="img-thumbnail" style={{ borderRadius: "0.5rem" }} />
                        </div>
                      ) : formData.photo ? (
                        <span className="photo-picker-name">
                          Image actuelle selectionnee
                        </span>
                      ) : (
                        <p className="photo-picker-text">Appuyer pour sélectionner une image</p>
                      )}
                    </div>
                  </label>
                </div>
            ...
            {!HIDDEN_FORM_FIELDS.has("designation") && (
              <div className="form-group-wrapper">
                <Form.Label className="form-label-enhanced">
                ...
                <Form.Control
                  ...
                  onChange={handleChange}
                  className={`form-control-enhanced ${validationErrors.designation ? "is-invalid" : ""}`}
                  placeholder="Entrez la designation"
                />
                {validationErrors.designation && <span className="error-message">{validationErrors.designation}</span>}
              </div>
            )}
            ...
            <div className="form-group-wrapper">
              ...
              <Form.Control
                ...
                onChange={handleChange}
                className={`form-control-enhanced ${validationErrors.numero_serie ? "is-invalid" : ""}`}
                placeholder="Entrez le numero de serie"
              />
              {validationErrors.numero_serie && <span className="error-message">{validationErrors.numero_serie}</span>}
            </div>

            {!HIDDEN_FORM_FIELDS.has("valeur") && (
              <div className="form-group-wrapper">
                <Form.Label className="form-label-enhanced">
            ...
            <div className="form-group-wrapper">
              ...
              <Form.Select
                ...
                value={formData.etat}
                onChange={handleChange}
                className={`form-control-enhanced ${validationErrors.etat ? "is-invalid" : ""}`}
              >
                <option value="">Selectionner</option>
              ...
              {validationErrors.etat && <span className="error-message">{validationErrors.etat}</span>}
            </div>

            {!HIDDEN_FORM_FIELDS.has("date_expiration") && (
              <div className="form-group-wrapper">
                <Form.Label className="form-label-enhanced">
            ...
            <div className="form-group-wrapper">
              ...
              <Form.Select
                ...
                onChange={handleChange}
                className={`form-control-enhanced ${validationErrors.statut ? "is-invalid" : ""}`}
              >
                {STATUTS_EQUIPEMENT.map((option) => (
                  <option key={option.value} value={option.value}>
                    {option.label}
                  </option>
                ))}
